...
def detect_drift(
    ...
    for column in columns:
        ...
        if df1[column].dtype == "object" or pd.api.types.is_categorical_dtype(
            df1[column]
        ):
            # chi-squared test for categorical variables
            contingency_table = pd.crosstab(df1[column], df2[column])
            if (contingency_table == 0).any().any():
                # ensure no zeros in contingency table
                contingency_table += 0.01
            ...
            # kolmogorov test for numeric variables
